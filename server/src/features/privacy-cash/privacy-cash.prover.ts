...
import type { ProofResult } from "./privacy-cash.types";
import { FIELD_SIZE, TRANSACT_IX_DISCRIMINATOR } from "./privacy-cash.constants";
const CIRCUIT_PATH = path.join(process.cwd(), "circuit", "transaction2");
const PROGRAM_ID = new PublicKey(config.privacyCash.programId);
export async function generateProof(input: any): Promise<ProofResult> {
